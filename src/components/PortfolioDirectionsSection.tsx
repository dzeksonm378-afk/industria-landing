    ...
    <Section eyebrow={content.eyebrow} title={content.title} description={content.description}>
      <div className="grid gap-5 md:grid-cols-2">
        {content.items.map((item) => (
          <Link key={item.title} href={item.href} aria-label={`${item.title}: открыть страницу направления`} className="group relative min-h-72 overflow-hidden rounded-lg border border-gold/30 focus-visible:outline-gold-soft">
            ...
            <div className="absolute inset-x-0 bottom-0 p-5 sm:p-7">
              ...
              <span className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-gold-soft">Подробнее о направлении <span aria-hidden="true">→</span></span>
            </div>
          </Link>
        ))}
      </div>
